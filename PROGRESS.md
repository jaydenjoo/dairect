...
### 세션 후반 — 대시보드 리드 CRM 기능 설명 (코드 변경 X)
Jayden 질문: "대시보드의 리드는 어떤 기능인지 알려줘". 코드베이스 직접 탐색 후 7개 핵심(소스 6종 / 상태 6단계 / 프로젝트 전환 / 필터 / 실패 추적) 정리해 답변. 코드 위치: `src/app/dashboard/leads/` + `schema.ts:133` + `validation/leads.ts`.

### ⚠️ 다음 세션 점검 후보 (이번 세션 답변 중 발견)
- **랜딩 "상담 신청하기" → leads 테이블 자동 INSERT 여부 점검** — 현재 CTA가 `/about#contact` 로 가는데, 그 폼이 `source=landing_form` 으로 자동 저장되는지 미확인. 안 되어 있으면 "랜딩 폼" 채널 사실상 무용지물 (수기 입력 필요) → 매출 누락 리스크.
