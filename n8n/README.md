...
- **W3** `weekly.summary` → PM 주간 요약 메일 (Gmail, Vercel Cron 매주 금요일 18:00 KST) — Phase 3 cron 전체 종결
- **W4** `project.completed` → 고객사 완료 안내 메일 (Gmail)
- **W5** `portal_feedback.received` → PM에게 고객 피드백 알림 메일 (Gmail) — Task 4-2 M7에서 신규 추가

구현 파일:
- 서버측: [`src/lib/n8n/client.ts`](../src/lib/n8n/client.ts), [`src/app/dashboard/projects/actions.ts`](../src/app/dashboard/projects/actions.ts), [`src/lib/portal/feedback-actions.ts`](../src/lib/portal/feedback-actions.ts), [`src/app/api/cron/invoice-overdue/route.ts`](../src/app/api/cron/invoice-overdue/route.ts), [`src/app/api/cron/weekly-summary/route.ts`](../src/app/api/cron/weekly-summary/route.ts)
- n8n 워크플로우: [`workflows/W1_project_status_changed.json`](workflows/W1_project_status_changed.json), [`workflows/W2_invoice_overdue.json`](workflows/W2_invoice_overdue.json), [`workflows/W3_weekly_summary.json`](workflows/W3_weekly_summary.json), [`workflows/W4_project_completed.json`](workflows/W4_project_completed.json), [`workflows/W5_portal_feedback_received.json`](workflows/W5_portal_feedback_received.json)
...
N8N_WEBHOOK_URL_WEEKLY_SUMMARY=https://n8n.example.com/webhook/dairect/weekly-summary
...
- ~~**W3** weekly reports 금요일 크론~~ — ✅ 완료 (Vercel Cron + `/api/cron/weekly-summary` + W3 JSON, 매주 금요일 KST 18:00)
...
## W3 — `weekly.summary` → PM 주간 요약 (Vercel Cron)

매주 금요일 KST 18:00 (UTC 09:00)에 Vercel Cron이 `/api/cron/weekly-summary`를 호출. `userSettings.businessEmail`이 있는 각 user에 대해 지난 7일 활동 8개 항목을 집계 → n8n `weekly.summary` 이벤트 발송 → `userSettings.lastWeeklySummarySentAt` 기록.

### 서버 측 cron endpoint (이미 구현)

- 위치: [`src/app/api/cron/weekly-summary/route.ts`](../src/app/api/cron/weekly-summary/route.ts)
- 인증: `Authorization: Bearer ${CRON_SECRET}` (W2와 시크릿 공유, `crypto.timingSafeEqual` 검증)
- 스케줄: [`vercel.json`](../vercel.json) — `0 9 * * 5` (금요일 UTC 09:00 = KST 18:00)
- 멱등성: `lastWeeklySummarySentAt < now - 6 days` 필터 (6일로 설정해 cron 지연 1일 여유). UPDATE WHERE에도 조건 재포함 → race 시 0 row.
- 빈 주(모든 count 합계 0) user는 메일 발송 skip — 피로도 방지
- 페이로드:
  ```json
  {
    ...
    "data": {
      ...
      "stats": {
        ...
        "newInvoices": 4,
        "paidInvoices": 3,
        "paidAmountTotal": "5000000",
        "paidAmountFormatted": "5,000,000",
...
### 집계 쿼리 (8개, `Promise.all` 병렬)

- **newProjects**: `projects.createdAt >= weekAgo` (count)
- **completedProjects**: `status='completed' AND updatedAt >= weekAgo` (count)
- **newEstimates**: `estimates.createdAt >= weekAgo` (count)
- **newInvoices**: `invoices.createdAt >= weekAgo` (count)
- **paidInvoices + paidAmountTotal**: `status='paid' AND paidDate >= weekAgoDate` (count + sum 단일 쿼리)
- **overdueInvoicesCount**: `status='overdue'` (현재 누적, 지난 주와 무관)
- **newLeads**: `leads.createdAt >= weekAgo` (count)
- **portalFeedbacks**: `projects JOIN` → `userId = u.id AND createdAt >= weekAgo` (count)

### DB 마이그레이션

`userSettings.last_weekly_summary_sent_at TIMESTAMPTZ NULL` 컬럼 추가. `src/lib/db/migrations/0016_charming_nemesis.sql`.

### n8n 워크플로 임포트

1. n8n UI → Import from File → [`workflows/W3_weekly_summary.json`](workflows/W3_weekly_summary.json)
2. **Gmail OAuth2 Credentials 연결** (W2/W4/W5와 동일 자격증명 재사용 가능)
3. **env 세팅**:
   - Vercel: `N8N_WEBHOOK_URL_WEEKLY_SUMMARY` + `CRON_SECRET` (W2와 공유)
   - n8n: 기존 `DAIRECT_WEBHOOK_SECRET` + (선택) `DAIRECT_DASHBOARD_BASE_URL`
4. 워크플로 Active 토글
...
```bash
curl -H "Authorization: Bearer ${CRON_SECRET}" https://dairect.kr/api/cron/weekly-summary
# 응답: { "total": N_users, "processed": N_sent, "failed": 0, "skipped": N_empty }
```

n8n Executions 탭:
- `Verify HMAC`: `verified=true`
- `Verified & Has pmEmail?`: true 분기
- `Compose Email`: 8개 stat 카드 HTML 생성 (현재 연체 > 0이면 "확인 필요" 서브라벨)
- `Gmail Send`: PM 1건 발송

### W3 내부 동작 (참고)

- **빈 주 스킵**: 지난 7일간 활동 전무인 user는 메일 발송 skip. 조용한 한 주 메일 피로도 방지.
- **HTML 카드 구조**: 8개 stat card (새 프로젝트 / 완료 / 신규 견적 / 신규 청구 / 입금 확인 + 총액 / 현재 연체 / 새 리드 / 포털 피드백) + 대시보드 CTA.
- **대시보드 링크**: `${DAIRECT_DASHBOARD_BASE_URL}/dashboard` — W2와 동일 패턴, trailing slash 자동 strip.
- **PII 방어**: `saveDataErrorExecution: "none"` — 실패 실행에도 pmEmail/금액/통계 저장 금지.
- **Deadline gate (Phase 5 multi-tenant 대비)**: `maxDuration 300s - 50s safety margin = 250s` 경과 시 루프 break. 미처리 user는 `lastWeeklySummarySentAt` 미갱신 → 다음 cron에서 자동 재개(재개 token 불필요). 응답 body에 `deadlineBroken: boolean` 포함.
- **BigInt-safe 금액 포맷**: `paidAmountTotal`은 PG numeric sum을 string으로 보존(`MAX_SAFE_INTEGER` 9,007조 초과 방어). n8n은 서버 pre-formatted된 `paidAmountFormatted` 직접 사용 — Number 변환 우회.
- **Phase 5 확장 포인트**: multi-PM 환경, 상세 차트 임베드(Recharts SVG → PNG), 전환율/마일스톤 지연 등 추가 stat.
