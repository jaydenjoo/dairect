...
- 발송 위치: `src/lib/portal/feedback-actions.ts` → INSERT 성공 후 fire-and-forget
- 페이로드:
  ```json
  {
...
### n8n 워크플로 구성 (Jayden 수동 작업)

구조는 **W4(`project.completed` Gmail 발송)와 동일**합니다. JSON 파일은 중복 유지 비용 대비 이득이 적어 저장소에 포함하지 않았습니다. **n8n UI에서 W4를 복제 후 아래 4개 노드를 모두 수정**해주세요. (W4 복제본에는 `Verify HMAC` → `If Verified` → `Respond 200` → `Compose Email` → `Gmail Send` 5개 노드가 있습니다. 그중 수정 필요한 것은 다음 4개.)

#### 1) Webhook 노드
- `path`: `dairect/portal-feedback-received`
- `webhookId`: `dairect-w5-portal-feedback-received` (새 UUID로 교체 — W4 값 유지 시 dead webhook)
- 워크플로 이름: `Dairect — W5 portal_feedback.received → Gmail`

#### 2) If 노드 (`Verified & Has Email?`)
W4는 `client_email` 필드를 확인하지만 W5는 `recipientEmail`입니다. 조건 변경:
```
{{ $json.body.data.recipientEmail }} is not empty
```

#### 3) Compose Email **Code 노드** (⚠️ 필수 수정)
W4의 Compose Email는 payload에서 발송용 데이터를 만들고 HTML escape를 수행합니다. **Gmail Send만 바꾸고 이 노드를 그대로 두면 빈 `to`로 발송 실패하거나 messagePreview의 특수문자가 escape되지 않습니다**. 노드의 jsCode를 아래로 **전체 교체**:
...
const d = ($json.body && $json.body.data) || {};
const to = stripCtrl(d.recipientEmail || '');
...
const dashboardUrl = `https://<your-dairect-domain>/dashboard/projects/${projectId}?tab=feedback`;
const html = `
  <p>"${escHtml(projectName)}" 프로젝트 포털에 새 피드백이 도착했습니다.</p>
  <pre style="background:#f5f5f5;padding:12px;border-radius:8px;white-space:pre-wrap;">${escHtml(preview)}</pre>
  <p>전체 내용과 읽음 처리는 대시보드에서 확인해주세요.</p>
  <p><a href="${escHtml(dashboardUrl)}">대시보드로 이동 →</a></p>
`.trim();

return [{ json: { to, subject, html } }];
```

- `<your-dairect-domain>`을 실제 배포 도메인으로 교체.
- `stripCtrl`로 `\r\n` 제거 → SMTP 헤더 injection 방어 (서버측 1차 방어 + n8n 측 2차 방어의 심층 방어).
- `escHtml`로 messagePreview의 HTML 특수문자 차단.
...
기존 W4의 바인딩이 이미 이 형태일 가능성이 높습니다 (Compose Email Code 노드의 출력 그대로 받음). 확인만 하세요.

#### 5) Workflow Settings
- `saveDataSuccessExecution: "none"` — PII 저장 금지 (기본값)
- `saveDataErrorExecution: "none"` — **반드시 `none`으로 변경** (W4 기본은 `all`이라 실패 실행 시 `recipientEmail + messagePreview`가 n8n DB에 영구 저장됨). 실패 디버깅은 별도 에러 워크플로를 두거나, PII strip된 필드만 로깅하는 에러 분기를 추가.

#### 6) 활성화 + 스모크
활성화 후 `/portal/[token]`에서 실제 피드백 제출 → n8n Executions 탭 `verified=true` + Gmail 발송 확인.

### 빠른 스모크 (앱 측)

```bash
# 개발 환경: .env.local에 N8N_WEBHOOK_URL_PORTAL_FEEDBACK_RECEIVED 설정
# 포털 페이지에서 피드백 제출 → n8n Executions 탭에서 Verify HMAC: verified=true 확인
```
